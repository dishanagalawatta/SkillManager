"""
Verification script for screenshot settings:
Starts real app, navigates to Settings view, captures screenshot of newly added Settings options.
"""
...
def step1_navigate_settings():
    ...
    # Scroll down the Settings view scroll area to expose the Projects screenshot options
    def scroll_settings():
        for child in window.findChildren(QObject):
            if child.metaObject().className() == "QQuickFlickable" or hasattr(child, "contentY"):
                try:
                    child.setProperty("contentY", 350.0)
                except Exception:
                    pass

    QTimer.singleShot(1500, scroll_settings)
    QTimer.singleShot(3500, step2_capture)


def step2_capture():
    print(f"[+{time.monotonic() - START_TIME:.1f}s] Capturing Settings view screenshot...")
    shot_path = str(CAPTURES_DIR / "verify_screenshot_settings.png")
