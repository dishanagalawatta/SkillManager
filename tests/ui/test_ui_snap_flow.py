...
class TestUISnapFlow:
    def test_full_screenshot_capture_and_save_flow(
        ...
        app_controller.config_controller.temporarySnaps = False
        ...
        # 2. Trigger capture
        with qtbot.waitSignal(app_controller.snap_controller.showOverlay, timeout=2000):
            app_controller.snap_controller.takeSnap()

        assert app_controller.snap_controller.current_full_pixmap is not None
        assert not app_controller.snap_controller.current_full_pixmap.isNull()
        ...
        # Save snap should emit captureFinished with the saved path
        with qtbot.waitSignal(
            app_controller.snap_controller.captureFinished, timeout=2000
        ) as blocker:
            app_controller.snap_controller.saveSnap(crop_rect, redactions)
        ...
        # The new snap should be added as a virtual skill to the models
        # It's a "Special" main category, "Snaps" category
        ...
        def check_model_for_screenshot():
            ...
            for i in range(model._all_skills.__len__()):
                ...
                if skill.get("is_snap", False) and skill.get("local_path") == saved_path:
                    return True
            return False
