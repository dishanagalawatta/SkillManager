...
def test_ops_controller_toggle_essential(ops_controller, mock_app):
    mock_app._selected_skill = {"local_path": "/path/e", "is_essential": False}
    with patch("skill_manager.controllers.ops_controller.save_essentials") as mock_save:
        ops_controller.toggle_essential()
        
        assert "/path/e" in mock_app._essential_paths
        assert mock_app._selected_skill["is_essential"] is True
        mock_save.assert_called_with(mock_app._essential_paths)

def test_ops_controller_toggle_essential_remove(ops_controller, mock_app):
    mock_app._selected_skill = {"local_path": "/path/e", "is_essential": True}
    mock_app._essential_paths = ["/path/e"]
    with patch("skill_manager.controllers.ops_controller.save_essentials") as mock_save:
        ops_controller.toggle_essential()
        
        assert "/path/e" not in mock_app._essential_paths
        assert mock_app._selected_skill["is_essential"] is False
        mock_save.assert_called_with(mock_app._essential_paths)
...
def test_ops_controller_cleanup_temp_copies(ops_controller, tmp_path):
    ...
    with patch("skill_manager.controllers.ops_controller.load_temp_registry", return_value=[str(temp_dir), str(temp_file)]), \
...
@patch("skill_manager.controllers.ops_controller.QTimer.singleShot")
@patch("threading.Thread")
def test_ops_controller_copy_selected_temporary(mock_thread, mock_timer, mock_copy, ops_controller, mock_app):
    def thread_side_effect(target, daemon=True):
        target()
        return MagicMock()
    mock_thread.side_effect = thread_side_effect
    mock_copy.return_value = {
        'copied': 1, 
        'merged': 0, 
        'details': [{'status': 'copied', 'message': '/target/S1'}]
    }
    
    mock_app.skillModel.getSelectedPaths.return_value = ["/p1"]
    mock_app.skillModel._all_skills = [{"local_path": "/p1", "name": "S1"}]
    
    with patch("skill_manager.controllers.ops_controller.load_temp_registry", return_value=[]), \
         patch("skill_manager.controllers.ops_controller.save_temp_registry") as mock_save:
        ops_controller.copy_selected_to_target("/target", is_temporary=True)
        
        mock_save.assert_called_with(["/target/S1"])
