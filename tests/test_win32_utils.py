...
def test_apply_native_style_windows_success_and_failure():
    ...
    with (
        ...
        apply_style.apply_style.side_effect = OSError("nope")
        win32.apply_native_style(MagicMock(), "mica")


def test_window_placement_windows_get_failure():
    with (
        patch("skill_manager.utils.win32.ctypes", create=True) as windll,
    ):
        windll.windll.user32.GetWindowPlacement.return_value = 0
...
def test_window_placement_windows_set_success():
    ...
    with (
        ...
        windll.windll.user32.SetWindowPlacement.return_value = 1
        assert win32.set_window_placement(123, placement_data) is True


def test_send_paste_to_focused_window_success():
    with (
        patch("skill_manager.utils.win32.injection_allowed", return_value=True),
        patch("skill_manager.utils.win32.ctypes", create=True) as windll,
    ):
        windll.windll.user32.keybd_event.return_value = None
        assert win32.send_paste_to_focused_window() is True
        assert windll.windll.user32.keybd_event.call_count == 4
        calls = windll.windll.user32.keybd_event.call_args_list
...
def test_send_paste_to_focused_window_failure():
    with (
        ...
        windll.windll.user32.keybd_event.side_effect = OSError("access denied")
