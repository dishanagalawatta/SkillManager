...
def sanitize_token(text: str | None) -> str | None:
    ...
    if "echo password=" in text:
        # Unified pattern that matches the entire shell token by combining unquoted characters,
        # single-quoted strings, and double-quoted strings, to prevent mixed-quote leaks.
        text = re.sub(
            r"(echo password=)((?:[^;\r\n\s'\"]|'[^']*'|\"(?:\\.|[^\"\\])*\")*)", r"\1***", text
        )
