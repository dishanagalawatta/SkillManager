...
class SkillIndexer:
    ...
    def build_index_data(self, skill: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "all_doc_tokens_set": set(all_doc_tokens),
...
class SearchEngine:
    ...
    def _calculate_score(self, query: str, index_data: dict[str, Any]) -> float:
        ...
        if query_tokens:
            all_doc_tokens_set = index_data.get("all_doc_tokens_set", set())

            if all_doc_tokens_set:
                max_token_match = 0

                # Fast path: Check exact match first using O(1) set operations
                # ⚡ Bolt: Use set.isdisjoint for ultra-fast O(1) matching bypassing nested loop
                if not all_doc_tokens_set.isdisjoint(query_tokens):
                    max_token_match = 100
                else:
