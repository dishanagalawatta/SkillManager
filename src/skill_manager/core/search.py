...
class SkillIndexer:
    ...
    def build_index_data(self, skill: dict[str, Any]) -> dict[str, Any]:
        ...
        # Pre-compute all_doc_tokens to avoid dynamic list concatenation in scoring loop
        all_doc_tokens = name_tokens + lower_tags + description_tokens
        if category_lower:
...
class SearchEngine:
    ...
    def query(
        ...
        # Hoist invariant tokenization out of the scoring loop
        ...
        for skill, index_data in self._indexed_data:
            if valid_paths is not None and skill.get("local_path") not in valid_paths:
                continue
            score = self._calculate_score(query_text, query_tokens, index_data)
            if score >= threshold:
    ...
    def _calculate_score(
        self, query: str, query_tokens: list[str], index_data: dict[str, Any]
    ) -> float:
        """
