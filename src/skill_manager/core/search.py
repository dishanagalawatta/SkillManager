    ...
    def _calculate_score(self, query: str, index_data: dict[str, Any]) -> float:
        ...
        if query_tokens:
            ...
            if all_doc_tokens:
                ...
                doc_tokens_set = set(all_doc_tokens)

                # Fast O(1) exact match check to bypass fuzzy matching
                if not query_tokens_set.isdisjoint(doc_tokens_set):
                    pass # We have an exact token match, proceed to full scoring
                else:
                    max_token_match = 0
