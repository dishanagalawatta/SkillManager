...
from skill_manager.core.models.entities import Skill, FilterState
...
class SkillModel(QAbstractListModel):
    ...
    def filterText(self): return self._state.filter_text
    @filterText.setter
    ...
    def collectionFilter(self): return self._state.collection_filter
    ...
    def projectFilter(self): return self._state.project_filter
    ...
    def clientFilter(self): return self._state.client_filter
    @clientFilter.setter
    def clientFilter(self, value):
        if self._state.client_filter != value:
            self._state.client_filter = value
            if self._state.filter_by_client: self._apply_filter()
            self._save_filters()
            self.clientFilterChanged.emit()

    @Property(bool, notify=filterByClientChanged)
    def filterByClient(self): return self._state.filter_by_client
    ...
    def showCommands(self): return self._state.show_commands
    @showCommands.setter
    ...
    def showStarred(self): return self._state.show_starred
    @showStarred.setter
    ...
    def isPackageOnly(self):
        if self._state.is_package_only is None: return Qt.PartiallyChecked
        return Qt.Checked if self._state.is_package_only else Qt.Unchecked
    @isPackageOnly.setter
    def isPackageOnly(self, value):
        new_val = None
        if value == Qt.Checked or value is True: new_val = True
        elif value == Qt.Unchecked or value is False: new_val = False
    ...
    def toggleSelection(self, row):
        if 0 <= row < len(self._filtered_skills):
            ...
            path = skill.local_path
            if not path or self._is_main_collapsed(skill) or self._is_sub_collapsed(skill): return
            if path in self._selected_ids: self._selected_ids.remove(path)
            else: self._selected_ids.add(path)
            idx = self.index(row, 0)
            self.dataChanged.emit(idx, idx, [self.IsSelectedRole])
            self.selectedCountChanged.emit(); self.userSelectionChanged.emit()
    ...
    def selectAll(self):
        for skill in self._filtered_skills:
            if self._is_main_collapsed(skill) or self._is_sub_collapsed(skill): continue
            if skill.local_path: self._selected_ids.add(skill.local_path)
        self._emit_selection_data_changed(); self.selectedCountChanged.emit()

    @Slot(result=list)
    def getSelectedPaths(self): return list(self._selected_ids)

    @Slot(list)
    def selectByPaths(self, paths):
        for path in paths:
            if path: self._selected_ids.add(path)
        self._emit_selection_data_changed()
        self.selectedCountChanged.emit(); self.userSelectionChanged.emit()
    ...
    def removeSkillsByPath(self, paths: list):
        ...
        self._apply_filter(); self.selectedCountChanged.emit()

    def _apply_filter(self, reset=False):
        if reset: self.beginResetModel()
        else: self.layoutAboutToBeChanged.emit()

        if self._state.filter_text and self._search_engine:
            valid_paths = {s.local_path for s in self._engine.filter_skills(self._all_skills, self._state)}
        ...
        self._filtered_skills = self._engine.build_visible_rows(self._all_filtered_skills, self._state.collapsed_categories)

        if reset: self.endResetModel()
        else: self.layoutChanged.emit()
        self.selectedCountChanged.emit()

    def _is_main_collapsed(self, skill: Skill):
        return (skill._main_category_name or self._engine.get_main_category(skill)) in self._state.collapsed_categories

    def _is_sub_collapsed(self, skill: Skill):
        return (skill._section_name or self._engine.get_section(skill)) in self._state.collapsed_categories

    def _emit_selection_data_changed(self):
        if not self._filtered_skills: return
        self.dataChanged.emit(self.index(0, 0), self.index(len(self._filtered_skills) - 1, 0), [self.IsSelectedRole])

    @Slot(int, result=dict)
    def get_skill_at(self, row):
        if 0 <= row < len(self._filtered_skills):
            # QML expects a dict or object it can access. 
            # dataclasses are accessible in QML if registered, but here we might need to return a dict for safety.
            import dataclasses
    ...
    def addOrUpdateSkills(self, new_skills: List[Dict[str, Any]]):
        ...
        self._search_engine = SearchEngine([s if isinstance(s, dict) else s.__dict__ for s in self._all_skills]) 
        self._apply_filter(reset=was_empty)
    ...
    def setSelected(self, row, selected):
        if 0 <= row < len(self._filtered_skills):
            ...
            if not path: return
            if selected: self._selected_ids.add(path)
            else: self._selected_ids.discard(path)
            idx = self.index(row, 0)
            self.dataChanged.emit(idx, idx, [self.IsSelectedRole])
            self.selectedCountChanged.emit()

    @Property(list, notify=collapsedCategoriesChanged)
    def collapsedCategories(self): return list(self._state.collapsed_categories)

    @Property(bool, notify=collapsedCategoriesChanged)
    def isAllExpanded(self): return len(self._state.collapsed_categories) == 0

    @Slot()
    def toggleAll(self): self.collapseAll() if self.isAllExpanded else self.expandAll()

    @Slot(str)
    def toggleCategory(self, name):
        if name in self._state.collapsed_categories: self._state.collapsed_categories.remove(name)
        else: self._state.collapsed_categories.add(name)
        self._save_collapsed(); self.collapsedCategoriesChanged.emit(); self._rebuild_visible_rows()
    ...
    def collapseAll(self):
        sections = { (s._main_category_name or self._engine.get_main_category(s)) for s in self._all_filtered_skills }
    ...
    def _rebuild_visible_rows(self):
        ...
        self._filtered_skills = self._engine.build_visible_rows(self._all_filtered_skills, self._state.collapsed_categories)
        self.layoutChanged.emit(); self.selectedCountChanged.emit()

    @Slot(str, result=bool)
    def isCategoryCollapsed(self, name): return name in self._state.collapsed_categories

    def _save_collapsed(self):
        if self._config: self._config.set("collapsed_categories", list(self._state.collapsed_categories))

    def _save_filters(self):
        if not self._config: return
        c = self._config; s = self._state
        c.set("show_archived", s.show_archived); c.set("category_filter", s.category_filter)
        c.set("collection_filter", s.collection_filter); c.set("project_filter", s.project_filter)
        c.set("client_format", s.client_filter); c.set("show_commands", s.show_commands)
        c.set("show_starred", s.show_starred); c.set("is_package_only", s.is_package_only)
