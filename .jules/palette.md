...
## 2024-05-26 - Missing Hover Cursors on Interactive Components
**Learning:** In Qt Quick Controls, standard `ComboBox` components and custom interactive elements like `FilterPill` do not automatically change the cursor on hover. This leads to a degraded mouse experience as users lack visual feedback that the element is clickable.
**Action:** Explicitly add a `HoverHandler { cursorShape: Qt.PointingHandCursor }` inside interactive components to visually indicate clickability.
