...
## 2026-06-02 - ComboBox Cursor Hover States
**Learning:** In Qt Quick Controls, standard `ComboBox` components (like custom dropdowns) do not automatically change the cursor on hover, which can obscure their interactability.
**Action:** Always add `HoverHandler { cursorShape: Qt.PointingHandCursor }` to custom `ComboBox` components to provide clear visual feedback to mouse users.
