...
**Learning:** Expand/collapse toggles are persistent on/off state (ADR-0031), so they require `Accessible.role: Accessible.CheckBox` with `checkable: true`, `checked` bound to the expanded state, BOTH `Accessible.onPressAction` and `Accessible.onToggleAction` wired to the same toggle slot, `activeFocusOnTab: true`, focus-visible styling (e.g. color bound to `activeFocus`), and `Keys.onPressed` handling `Space`/`Enter`/`Return` with `event.accepted = true` (plus `else { event.accepted = false }`). A `Button` role announces no state and hides the AT action when handlers are missing.
