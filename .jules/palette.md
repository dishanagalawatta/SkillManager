...
## 2024-05-31 - Window Resize Handle Accessibility
**Learning:** In QML frameless windows, `MouseArea` elements used as resize handles must include `Accessible.role: Accessible.Grip` along with appropriate `Accessible.name` and `Accessible.description` properties for screen reader accessibility. By default, they lack semantic meaning.
**Action:** Always verify that structural interactive items like resize edges or corners have appropriate accessibility metadata, specifically using `Accessible.Grip`.
