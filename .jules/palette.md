...
## 2026-06-20 - Unintended File Modifications During Build
**Learning:** Executing local test commands or scripts may sometimes trigger unintended modifications to lockfiles (like `uv.lock`) or binary assets (like `assets/brand/logo.ico`).
**Action:** Always review the `git status` or unstaged changes, and explicitly `git restore` unrelated files before creating a patch or committing, keeping the changes focused solely on the intended objective.
