...
## 2024-05-26 - Custom QML Item Focus Accessibility (Switches and CheckBoxes)
**Learning:** Custom interactive QML components built on plain `Item` or `Rectangle` (like `GlassSwitch` and `GlassCheckBox`) are not reachable via keyboard navigation by default. Additionally, because they don't inherit from `QtQuick.Controls.Control`, they lack a `visualFocus` property.
**Action:** Always add `activeFocusOnTab: true` to the root `Item` of custom controls, map `activeFocus` (NOT `visualFocus`) to visual indicators like border width and color, and handle `Keys.onPressed` for standard activation keys (Space, Enter) so users can trigger them via keyboard.
