...
## 2026-06-13 - [Overly Greedy Regex in Process Log Sanitization]
**Vulnerability:** The application sanitizes `echo password=...` strings from log outputs using `re.sub(r"(echo password=).*", r"\1***", text)`. The greedy `.*` matcher consumes the entire rest of the line. If a developer or a package manager script chains commands like `echo password=secret; execute_next_step`, the `execute_next_step` part is completely destroyed and hidden from the log output, complicating debugging and hiding potentially malicious chained commands.
**Learning:** Using greedy matchers like `.*` to redact inline secrets is unsafe for structured shell commands. It not only destroys legitimate surrounding output but also fails to handle the nuances of quoted vs unquoted tokens, often leaking secrets if quotes are escaped or improperly terminated.
**Prevention:** Use quote-aware regex patterns when sanitizing inline shell assignments (e.g. `r'(echo password=)"(?:\\.|[^"\\])*"'` for double quotes). Ensure unquoted fallback matchers explicitly stop at whitespace, semicolons, or newlines instead of blindly consuming to the end of the line.
