...
## 2026-05-27 - [Argument Injection in System Shell Commands on Windows]
**Vulnerability:** In cross-platform utility code, user-provided command strings were being evaluated via `subprocess` with `shell=True` as a fallback or by passing raw strings directly when `shell=False`. On Windows, passing a raw string to `shell=False` causes Python to bypass argument tokenization and execution of `.cmd`/`.bat` files often fails or executes insecurely.
**Learning:** `shell=True` is universally dangerous, but directly swapping it to `shell=False` with a raw string on Windows breaks functionality and does not safely tokenize arguments. Python provides `shlex.split`, but its behavior varies by platform.
**Prevention:** To safely eliminate `shell=True` without breaking cross-platform execution (especially Windows), parse the command string into a list using `shlex.split(command, posix=sys.platform != 'win32')` and pass the resulting list to `subprocess.Popen` or `run` with `shell=False`.
