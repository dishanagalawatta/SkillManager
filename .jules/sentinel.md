...
## 2026-07-29 - [Unsafe Temporary File Creation via tempfile.mktemp]
...
## 2024-05-20 - [Command Injection via Unsanitized Shell String Interpolation in Echo Statements]
**Vulnerability:** A command injection vulnerability was present when generating `verify_command`s. Although paths were correctly `shlex.quote`d for the main command (e.g., `test -d {quoted_path}`), the unquoted path (`{expanded}`) was still being directly interpolated inside a double-quoted `echo` string (`echo "Skills installed in {expanded}"`). This could allow arbitrary shell execution if the path contained quotes or shell metacharacters.
**Learning:** `shlex.quote` must be applied to all instances where a variable is interpolated into a shell string, even if it is enclosed inside double quotes in an `echo` statement, as double quotes do not prevent all shell expansions and can be easily broken out of.
**Prevention:** Always use `shlex.quote(expanded)` for all instances of a variable being formatted into a shell string execution or configuration field.
