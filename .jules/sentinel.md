...
## 2026-07-06 - [Argument Injection Bypass via shell=False and raw string on Windows]
**Vulnerability:** In `updater.py`, passing a raw command string to `run_process(..., shell=False)` on Windows securely invokes `CreateProcess` directly, circumventing potential `shell=True` meta-character injections. However, this causes severe functional regressions because `CreateProcess` (unlike `cmd.exe` or `shutil.which`) does not automatically resolve `.cmd` or `.bat` extensions. As a result, critical executables like `npm` or `npx` (which are `.cmd` files on Windows) immediately fail with `FileNotFoundError`.
**Learning:** Hardening cross-platform command execution by forcing string commands into `shell=False` on Windows breaks `.cmd` resolution. Safely converting string commands into lists using `shlex.split` across *all* platforms lets `subprocess` dynamically resolve executables while still avoiding `shell=True`.
**Prevention:** Instead of trying to maintain unsafe raw strings for Windows `shell=False` fallbacks, refactor functions wrapping `subprocess` to always accept and route `list[str]`. This maintains cross-platform safety and relies cleanly on the underlying `shutil.which` implementation for extensions like `.cmd` resolution.
