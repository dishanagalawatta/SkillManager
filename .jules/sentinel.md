...
## 2024-06-25 - Fix shell token secret redaction
**Vulnerability:** The regex used to redact secrets in `echo password=...` log lines failed to properly handle passwords containing mixed quotes (e.g., `'secret'"'"'more'`), resulting in the first quoted segment being redacted but leaving the remainder of the password exposed in plain text.
**Learning:** When using regular expressions to redact secrets embedded within shell commands, avoid using separate patterns for quoted and unquoted segments, as this fails to fully redact mixed-quote tokens (e.g., `'secret'"'"'more'`). Instead, use a unified pattern that matches the entire shell token by combining unquoted characters, single-quoted strings, and double-quoted strings (e.g., `r"(?:[^;\r\n\s'\"]|'[^']*'|\"(?:\\.|[^\"\\])*\")+"`).
**Prevention:** Ensure that regex redaction applied to shell commands parses entire shell tokens rather than naive quoted substrings to prevent partial redaction and leakage of complex passwords.
