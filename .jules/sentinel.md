...
## 2026-07-04 - [Arbitrary Command Execution via shell=True]
**Vulnerability:** The application executed user-provided configuration strings (`update_command` and `verify_command` via `run_shell_command` in `src/skill_manager/core/skill_packages/updater.py`) using `subprocess.Popen(..., shell=True)` (via `run_process`). This allowed attackers to execute arbitrary shell commands (e.g. `&& malicious_command`) if they controlled the configuration data.
**Learning:** Using `shell=True` with user-controlled configuration strings is fundamentally unsafe, as the shell evaluates metacharacters and executes them as separate commands. It is always necessary to tokenize shell strings safely and use `shell=False`.
**Prevention:** Convert user-provided shell command strings to lists using `shlex.split(command, posix=sys.platform != "win32")` and execute them with `shell=False` to prevent shell interpretation of metacharacters.
