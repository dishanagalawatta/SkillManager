...
## 2026-05-27 - [Argument Injection in `shell=True` Configuration Commands]
**Vulnerability:** User-controlled configuration parameters (`update_command` and `verify_command` in package config) were directly executed via `run_process(..., shell=True)` without proper sanitization. This allows arbitrary shell command execution if the config is maliciously crafted.
**Learning:** Functions that accept strings and pass them down to `shell=True` execution contexts are inherent command injection vulnerabilities, especially when those strings originate from editable user configurations (like `.skill-lock.json` or imported package metadata).
**Prevention:** Avoid `shell=True` entirely. When executing arbitrary configuration commands while maintaining compatibility with Windows `.cmd`/`.bat` files, parse the command string into a list using `shlex.split(command, posix=sys.platform != 'win32')` and execute it with `shell=False`.
