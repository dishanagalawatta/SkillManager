...
## 2026-06-25 - [Arbitrary Command Execution via shell=True in Skill Updates]
**Vulnerability:** The `run_shell_command` function in `src/skill_manager/core/skill_packages/updater.py` executed parsed update commands using `subprocess` with `shell=True`. Since `source["update_command"]` can be modified or injected via manipulated metadata in untrusted skills, an attacker could execute arbitrary commands through shell metacharacters (e.g., `;`, `&`, `|`).
**Learning:** Functions named "shell_command" often misleadingly imply that `shell=True` is required. In this case, cross-platform pseudo-shell commands like `test -d ... && echo ...` were already safely intercepted by `intercept_cross_platform_command`. The remaining use cases did not require shell features and should be executed safely as lists.
**Prevention:** Avoid `shell=True` entirely. Parse dynamic command strings safely using `shlex.split(command)` and execute them with `shell=False`.
